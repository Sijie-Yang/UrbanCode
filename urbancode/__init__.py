...
def __getattr__(name):
    ...
    Use importlib.import_module for subpackages. ``from . import svi`` inside
    __getattr__ re-enters getattr for ``svi`` and recurses infinitely (PEP 562).
    """
    if name == 'svi':
        try:
            svi = importlib.import_module('.svi', __name__)
        except ImportError as e:
            raise ImportError(f"Failed to import svi module: {e}") from e
        globals()['svi'] = svi
        return svi
    ...
    if name in network_attrs:
        try:
            globals()['network'] = importlib.import_module('.network', __name__)
            from .network.core import (
                download_network,
                save_network,
                load_saved_network,
